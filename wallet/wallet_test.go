...
func TestBIP86TaprootDerivation(t *testing.T) {
	...
	t.Run("BIP86TestVectors", func(t *testing.T) {
		...
		assert.Equal(t, "bc1p5cyxnuxmeuwuvkwfem96lqzszd02n6xdcjrs20cac6yqjjwudpxqkedrcr", firstReceive.Address)
		...
		assert.Equal(t, "bc1p4qhjn9zdvkux4e44uhx8tc55attvtyu358kutcqkudyccelu0was9fqzwh", secondReceive.Address)
		...
		assert.Equal(t, "bc1p3qkhfews2uk44qtvauqyr2ttdsw7svhkl9nkm9s9c3x4ax5h60wqwruhk7", firstChange.Address)
	})
	...
	t.Run("IndexIncrement", func(t *testing.T) {
		...
		assert.True(t, newReceiveAddr.Address[:4] == "bc1p", "Should be a taproot address starting with bc1p")
		...
		assert.True(t, newChangeAddr.Address[:4] == "bc1p", "Should be a taproot address starting with bc1p")
	})
	...
	t.Run("ScriptPubKeyGeneration", func(t *testing.T) {
		...
		receiveScriptPubKey, err := wallet.ReceiveScriptPubKey()
		assert.NoError(t, err)
		...
		changeScriptPubKey, err := wallet.ChangeScriptPubKey()
		assert.NoError(t, err)
